...
public class OVXApplication {
    ...
    public OVXApplication(NetCfg netCfg) {
        ...
        this.vCostMap = new HashMap<String, Long>();
        this.eCostMap = new HashMap<Edge, Long>();
        this.eAvailableResourceMap = new HashMap<>();
        this.vAvailableResourceMap = new HashMap<>();
        // vertex
...
//            }
            ...
            resourceAllocation = new ResourceAllocation(resourceRequest, vAllocationMapCache, eAllocationMapCache, isAllocateSuccess);
            long totalCost = 0L;
            for (String virtualVertex : vAllocationMapCache.keySet()) {
            ...
            for (Edge virtualEdge : eAllocationMapCache.keySet()) {
        ...
        public boolean allocateInternal() {
            ...
            for (String physicalvertex : availablePhysicalvertexList) {
                ...
                for (final Edge virtualEdge : relatedVirtualEdgeList) {
                    ...
                    long vRequest = resourceRequest.getEdgeRequest(virtualEdge);
                    ...
                    // Create a new graph that only include links which meet request
                    UndirectedGraph newGraph = new SimpleGraph(Edge.class);
                    for (Object v : physicalGraph.vertexSet()){
                        newGraph.addVertex(v);
                    }
                    for (Object e : physicalGraph.edgeSet()) {
                        if (eAvailableResourceMapCache.get(e) >= vRequest) {
                            Edge edge = (Edge) e;
                            newGraph.addEdge(edge.getSource(), edge.getTarget());
                        }
                    }
                    DijkstraShortestPath djPathService = new DijkstraShortestPath(newGraph);
...
//                        if (eAvailableResourceMapCache.get(edge) < vRequest) {
//                            isPhysicalVertexAvailable = false;
//                            break;
//                        }
//                    }
                    if (!isPhysicalVertexAvailable) break;
                    // Resort edge in path
            ...
            if (lastAvPhyVertexList.isEmpty()) return false;
